...
export async function POST(request) {
  ...
  try {
    ...
    // (FIXED: Now uses seller's stored location FIRST, then API fallback)
    let sellerLocation = null;

    // ------------------------ FUTURE LOGIC (NOW FIXED & ENABLED) ------------------------
    // NOW using seller's stored Shiprocket location FIRST (pickup location ID)
    if (sellerId) {
      try {
        ...
        if (sellerDoc.exists) {
          ...
          if (sellerData.shiprocketPickupLocation) {
            sellerLocation = sellerData.shiprocketPickupLocation; // This is the NUMERIC ID saved at registration
            console.log("Using seller's saved Shiprocket pickup location:", sellerLocation);
          }
        }
      } catch (error) {
    ...
    // If seller has no saved location → fallback to API
    if (!sellerLocation) {
      try {
        ...
        if (pickupLocations && pickupLocations.length > 0) {
          // FIX: Shiprocket requires the pickup LOCATION ID (not the name string)
          sellerLocation = pickupLocations[0].id;
          console.log('Using first ShipRocket pickup location ID:', sellerLocation);
        } else {
          // Hardcode the actual ShipRocket location from test results
          // FIX: This must be the ID, NOT `"Primary"`
          sellerLocation = 16235370; // Known working ID
          console.log('No locations from API, using hardcoded fallback ID:', sellerLocation);
        }

      } catch (error) {
        console.error('Error fetching ShipRocket locations:', error);
        // Hardcode the actual ShipRocket location from test results
        // FIX: Must be ID, NOT `"Primary"`
        sellerLocation = 16235370;
        console.log('API failed, using hardcoded location ID:', sellerLocation);
      }
    }
    ...
    const shiprocketPayload = {
      ...
      // FIX: pickup_location must be NUMERIC ID, not the string "Primary"
      pickup_location: Number(sellerLocation), // Now using seller's actual location (ID)
    ...
    await orderRef.update({
      ...
      pickup_location: sellerLocation, // Store which location was used (ID)
      updatedAt: new Date()
    });
